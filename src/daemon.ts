...
async function cleanWithOllama(text: string): Promise<string> {
  ...
  const prompt = `
Act as a professional editor.  Respond ONLY with the output text.
Please clean up the following raw transcript generated by speech-to-text software. Apply the following rules:

1. **Remove Filler Words:** Remove vocalized pauses (e.g., "um," "uh," "like," "you know," "sort of") to improve flow.
2. **Fix Punctuation & Capitalization:** Add necessary punctuation (periods, commas, question marks) and ensure proper capitalization. Break the text into logical paragraphs for readability.
3. **Correct Misrecognitions:** If an obvious error was made due to homophones or improper voice-to-text recognition (e.g., "accept" instead of "except" or technical terms), correct it based on the context.
4. **Maintain Tone & Meaning:** Do not change the original message, tone, or key facts. Keep the language natural and, where appropriate, conversational, but polished.
5. **Format Numbers:** Replace spelled-out numbers (e.g., "five thousand" to 5,000).
